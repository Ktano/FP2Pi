...
def e_especificacao(arg):
    ...
    if not isinstance(arg,tuple):
        ...
        return False
    elif not e_esp_LC(arg[0]) or not e_esp_LC(arg[1]):
        return False
    else:
        return True
...
def tabuleiro_celula(t,c):
    ...
    if e_tabuleiro(t) and e_coordenada(c):
        ...
        if coordenada_valida(dim,c):
            if coordenada_para_cadeia(c) in t['tab']:
                return t['tab'][coordenada_para_cadeia(c)]
            else:
                return 0
...
def tabuleiro_preenche_celula(t,c,v):
    ...
    if e_tabuleiro(t) and e_coordenada(c) and v in (0,1,2):
        ...
        if coordenada_valida(dim,c):
            if v in (1,2):
                t["tab"][coordenada_para_cadeia(c)]=v
            elif c in t["tab"]:
                del t[coordenada_para_cadeia(c)]
        else: 
...
def e_jogada (arg):
    ...
    if isinstance(arg, (tuple)):
        if len(arg)==2: 
            if e_coordenada(arg[0]) and arg[1] in [1,2]:
                return True
...
def celulas_vazias(t):
    ...
    for l in range(1,dim[0]+1):
        for c in range(1,dim[1]+1):
            val_celula=tabuleiro_celula(t, cria_coordenada (l, c) )
            if val_celula == 0:
                tuplo_controlo+=(cria_coordenada (l, c),)
    return tuplo_controlo
